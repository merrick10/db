# -*- coding:utf-8 -*-
import os,sys,logging,re,string,zipfile,shutil
#mysql_x64安装
#替换my.ini其中的绝对路径
...
path_mysqlini_bak = mainfolder + '/mysql_x64/my.ini_bak'
path_myini = mainfolder + '/mysql_x64/my.ini'
...
executeScript(os.path.join( currentdir,'./db_install_x64.bat'))

#解压war文件
logging.info('Start uncompress .war file:')
##shutil.rmtree(os.path.join(mainfolder,'./tomcat7_x64/webapps/czjpcoms'))
##logging.info('Delete folder success:'+os.path.join(mainfolder,'./tomcat7_x64/webapps/czjpcoms'))
warfile = zipfile.ZipFile(mainfolder+'/czjpcoms.war')
warfile.extractall(mainfolder+'/tomcat7_x64/webapps/czjpcoms')
warfile.close()
logging.info('End uncompress .war file:')

#替换tomcat_install.bat其中路径
path_tomcatbat_install =  currentdir + '/tomcat_install_x64.bat'
path_tomcatbat_install_bak =  currentdir + '/tomcat_install_x64.bat_bak'
createNewScript(path_tomcatbat_install_bak, path_tomcatbat_install,'<setupdir>',mainfolder)

#替换Tomcat/bin/service.bat其中路径
path_servicebat = mainfolder + '/tomcat7_x64/bin/service.bat'
path_servicebat_bak = mainfolder + '/tomcat7_x64/bin/service.bat_bak'
createNewScript(path_servicebat_bak, path_servicebat,'<setupdir>',mainfolder)

#替换wepapp日志路径
#${tomcatwebapps}/czjpcoms/WEB-INFclasses/log4jex.xml
path_log4j = mainfolder + '/tomcat7_x64/webapps/czjpcoms/WEB-INF/classes/log4jex.xml'
...
path_prop = mainfolder + '/tomcat7_x64/webapps/czjpcoms/WEB-INF/classes/com/czjpcoms/utils/settings.properties'
...
#执行tomcat服务安装
executeScript( os.path.join( currentdir,'./tomcat_install_x64.bat'))
